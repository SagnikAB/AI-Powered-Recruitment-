...
from core.ml_model         import ml_score_resume          # ← semantic ML scorer
...
def analyze():
    try:
        ...
        # ── Scoring pipeline ──────────────────────────────────────────────────
        # 1. Keyword-based score (matched/missing keyword breakdown)
        kw_score, matched, missing = score_resume(text, job_description)

        # 2. Semantic ML score (sentence-transformers or TF-IDF fallback)
        sem_score = ml_score_resume(text, job_description)

        # 3. Pinecone vector similarity (0-100), 0 if Pinecone not configured
        try:
        ...
        # ── Blend scores ──────────────────────────────────────────────────────
        # Weights: keyword 40 % | semantic 40 % | vector 20 %
        # If Pinecone is disabled (vector_score == 0), redistribute to 50/50.
        if vector_score > 0:
            final_score = int(kw_score * 0.40 + sem_score * 0.40 + vector_score * 0.20)
        else:
            final_score = int(kw_score * 0.50 + sem_score * 0.50)

        final_score = max(0, min(100, final_score))

        # Store for future similarity queries (best-effort)
        try:
...
# ═══════════════════════════════════════════════════════════════════
# INTERVIEW — Step 1: generate questions
# ═══════════════════════════════════════════════════════════════════
@app.route("/api/interview/questions", methods=["POST"])
def get_interview_questions():
    """
    Body: { "resume_id": 5, "matched_skills": ["python","sql",...] }
    Returns interview questions (ideal answers stripped for the frontend).
    """
    try:
        data           = request.get_json()
        if not data:
        ...
        # Strip ideal answers before sending to the frontend
        questions_clean = [
        ...
        return jsonify({
            ...
            "full":      questions,   # frontend caches this; sent back on /submit
            "resume_id": resume_id,
        })
...
def submit_interview():
    ...
    Body: {
        ...
        "ats_score":      72,
        "matched_skills": [...],
        "questions":      [...full question objects with ideal answers...],
        "answers":        {"0": "...", "1": "...", ...}
    }
    """
    try:
